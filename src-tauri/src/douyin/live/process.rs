...
pub async fn process_messages(
    ...
    let _contents: Vec<String> = messages.iter().map(|m| m.method.clone()).collect();
    ...
    for msg in messages {
        match msg.method.as_str() {
            ...
            // 聊天消息
            "WebcastChatMessage" => {
                // 你给出的 un_pack_webcast_chat_message 已经是 async fn
                if let Err(e) =
                    message::un_pack_webcast_chat_message(app, task_id, &msg.payload).await
                {
                    eprintln!("un_pack_webcast_chat_message error: {}", e);
                }
            }
            // 反对分数消息
